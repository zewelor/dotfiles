...
alias update="\sudo apt autoremove -y --purge && \sudo apt update && \sudo apt full-upgrade -y && \sudo apt autoremove -y --purge$(has "flatpak" && echo " && flatpak update -y") $(has "snap" && echo " && snap refresh") $(has "pipx" && echo " && sudo -u omen pipx upgrade-all --include-injected")"
